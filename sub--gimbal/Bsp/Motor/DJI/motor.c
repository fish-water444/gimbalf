...
{
    ptr->dt = DWT_GetDeltaT(ptr->MotorDwtCount);
    // ���C620����ֲ�
    ...
    if (ptr->dt != 0)
    {
        ptr->Acceleration = (ptr->Velocity_RPM - ptr->Last_Velocity_RPM) / ptr->dt;
        ptr->AccelerationFiltered = ptr->Acceleration * ptr->dt / (ptr->AccelerationLpf + ptr->dt) +
                                    ptr->LastAcceleration * ptr->AccelerationLpf / (ptr->AccelerationLpf + ptr->dt);

        ptr->AccelDerivatives = (ptr->AccelerationFiltered - ptr->LastAcceleration) / ptr->dt;
    }
    ptr->Angle = loop_float_constrain(ptr->RawAngle - ptr->zero_offset, -4095, 4096);

    ptr->last_AngleInDegree = ptr->AngleInDegree;
    ...
    // ptr->LastAccelDerivatives = ptr->AccelDerivatives;
    ptr->Last_Velocity_RPM = ptr->Velocity_RPM;
    ptr->LastAcceleration = ptr->AccelerationFiltered;
}
