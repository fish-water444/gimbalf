...
#define RMD9025V1_CURRENT_COEF 407.6f
#define RMD9025V2_CURRENT_COEF 402.7f
...
typedef struct motor_t
{
    ...
    float AccelerationLpf;
    float LastAcceleration;

    float AccelDerivatives;
    float LastAccelDerivatives;
    float Real_Current;
    uint8_t Temperature;
    float dt;
    uint32_t *MotorDwtCount;
...
void get_C_moto_info(Motor_t *ptr, Motor_t *ptr_, uint8_t *aData);

HAL_StatusTypeDef Send_RMD_Current(CAN_HandleTypeDef *_hcan, int16_t c1, int16_t c2, int16_t c3, int16_t c4);
HAL_StatusTypeDef Send_RMD_Current_Single(CAN_HandleTypeDef *_hcan, int8_t ID, int16_t c);
