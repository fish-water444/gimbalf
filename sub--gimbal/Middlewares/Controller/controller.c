...
static void f_PID_ErrorHandle(PID_t *pid)
{
    ...
    if (pid->ERRORHandler.ERRORCount > 100) // 500ms
    {
...
float Yaw_FrictionFF(Yaw_FrictionFF_Params_t *params, float v_ref)
{
    if (params == NULL)
        return 0;
    ...
    if (params->v_eps <= 1e-2)
        params->v_eps = 1e-2;


    // 方向：看参考速度，避免实际速度零点噪声来回翻
    float dir = tanhf(v_ref / params->v_eps);

    // 幅值：按当前运动速度大小衰减
    float v_abs = fabsf(v_ref);

    // 计算输出
    float mag = params->I_c + (params->I_s - params->I_c) * expf(-(v_abs / params->v_s) * (v_abs / params->v_s));
