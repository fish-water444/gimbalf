...
static void f_PID_ErrorWrapping(PID_t *pid);
    ...
    uint16_t improve)
{
    ...
    pid->warpping_period = warpping_period;
...
float PID_Calculate(PID_t *pid, float measure, float ref)
{
    ...
    if (pid->Improve & ErrorWrapping)
        f_PID_ErrorWrapping(pid);
...
static void f_PID_ErrorHandle(PID_t *pid)
{
    ...
    if ((fabsf(pid->Ref - pid->Measure) / fabsf(pid->Ref)) > 0.95f &&
        fabsf(pid->Output) > pid->MaxOut * 0.95f)
    {
    ...
    if (pid->ERRORHandler.ERRORCount > 100) // 100
    {
...
static void f_PID_ErrorWrapping(PID_t *pid)
{
    float half_period = pid->warpping_period / 2.0f;
    
    pid->Err = loop_float_constrain(pid->Err, -half_period, half_period);
}
