...
uint8_t shoot_data_count = 0;      // 判断射击数据是否更新
uint8_t last_shoot_data_count = 0; // 判断射击数据是否更新
...
radar_interaction_info_t radar_interaction_info;     // 雷达交互信息

KeyboardMouseControl_t keyboard_mouse_control;                          // 键鼠控制数据
CustomControl_t custom_control;                                         // 自定义控制数据
GameStatus_t custom_game_status;                                        // 比赛全局状态
GlobalUnitStatus_t global_unit_status;                                  // 基地/前哨站/机器人状态
GlobalLogisticsStatus_t global_logistics_status;                        // 全局后勤信息
GlobalSpecialMechanism_t global_special_mechanism;                      // 全局特殊机制
Event_t custom_event;                                                   // 全局事件通知
RobotInjuryStat_t robot_injury_stat;                                    // 机器人一次存活期间累计受伤统计
RobotRespawnStatus_t robot_respawn_status;                              // 机器人复活状态
RobotStaticStatus_t robot_static_status;                                // 机器人固定属性和配置
RobotDynamicStatus_t robot_dynamic_status;                              // 机器人实时数据
RobotModuleStatus_t robot_module_status;                                // 机器人模块状态
RobotPosition_t robot_position;                                         // 机器人空间坐标和朝向
Buff_t custom_buff;                                                     // Buff 效果信息
PenaltyInfo_t penalty_info;                                             // 判罚信息同步
RobotPathPlanInfo_t robot_path_plan_info;                               // 哨兵轨迹规划信息
MapClickInfoNotify_t map_click_info_notify;                             // 小地图点击信息通知
RadarInfoToClient_t radar_info_to_client;                               // 雷达发送的机器人位置信息
CustomByteBlock_t custom_byte_block;                                    // 自定义数据流
AssemblyCommand_t assembly_command;                                     // 工程装配指令
TechCoreMotionStateSync_t tech_core_motion_state_sync;                  // 科技核心运动状态同步数据
RobotPerformanceSelectionCommand_t robot_performance_selection_command; // 地面机器人选择性能体系/控制方式
RobotPerformanceSelectionSync_t robot_performance_selection_sync;       // 性能体系状态同步
CommonCommand_t common_command;                                         // 通用控制数据
HeroDeployModeEventCommand_t hero_deploy_mode_event_command;            // 英雄部署模式指令
DeployModeStatusSync_t deploy_mode_status_sync;                         // 英雄部署模式状态同步
RuneActivateCommand_t rune_activate_command;                            // 能量机关激活指令
RuneStatusSync_t rune_status_sync;                                      // 能量机关状态同步
SentryStatusSync_t sentry_status_sync;                                  // 哨兵姿态和弱化状态
DartCommand_t dart_command;                                             // 飞镖控制指令
DartSelectTargetStatusSync_t dart_select_target_status_sync;            // 飞镖目标选择状态同步
SentryCtrlCommand_t sentry_ctrl_command;                                // 哨兵控制数据
SentryCtrlResult_t sentry_ctrl_result;                                  // 哨兵控制结果数据
AirSupportCommand_t air_support_command;                                // 空中支援数据
AirSupportStatusSync_t air_support_status_sync;                         // 空中支援状态反馈

void Judge_Control_Init(UART_HandleTypeDef *huart)
{
    JudgeUSART = huart;
    USART_IDLE_Init(huart, judgement_receive.buf, 254);
}
